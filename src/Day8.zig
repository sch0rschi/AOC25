...
const UnionFind = struct {
    depth: u8 = 1,
    size: u16 = 1,
    parent_index: u16 = NUMBER_OF_NODES,
};
...
pub fn main() void {
    ...
    for (0..NUMBER_OF_NODES) |i| {
        ...
        for (i + 1..NUMBER_OF_NODES) |j| {
            const to = nodes[j];
            edges[offset].from = @intCast(i);
            edges[offset].to = @intCast(j);
    ...
    var pq = PriorityQueue(Edge, less_weight).init(&edges);
    pq.heapify();

    var connected: usize = 0;
    var union_find_from_root_index: u16 = undefined;
    var union_find_to_root_index: u16 = undefined;
    for (0..PART_1_STOP) |_| {
        const edge = pq.pop();
        union_find_from_root_index = find_root(edge.from, &union_find);
        union_find_to_root_index = find_root(edge.to, &union_find);
    ...
    while (connected < NUMBER_OF_NODES - 1) {
        edge = pq.pop();
        union_find_from_root_index = find_root(edge.from, &union_find);
        union_find_to_root_index = find_root(edge.to, &union_find);
        if(union_find_from_root_index == union_find_to_root_index) {
...
fn less_weight(a: Edge, b: Edge) bool {
    return a.weight < b.weight;
}

inline fn find_root(index: u16, unionFind: *[NUMBER_OF_NODES]UnionFind) u16 {
    var current = unionFind[index];
    var current_index: u16 = index;
